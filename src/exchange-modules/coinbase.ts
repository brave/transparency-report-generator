...
async function _getOrders(
  ...
  const orders = (await response.json()) as CoinbaseOrder[];

  debugLOG(`Coinbase returned ${orders.length} orders.`);
...
function isValidOrder(order: CoinbaseOrder, since: number): boolean {
  const done_at = new Date(order.done_at).getTime();
  const { settled, side, status, product_id } = order;
  return (
    settled &&
    side == "buy" &&
    status == "done" &&
